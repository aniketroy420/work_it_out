...
class HiveDatabase {
  ...
  bool previousDataExists() {
    if (_myBox.isEmpty) {
      print("previous data doesn't not exist");
      _myBox.put("START_DATE", todaysDDMMYYYY());
      return false;
    } else {
   ...
   */
    ...
    if (exerciseCompleted(workouts)) {
      _myBox.put("COMPLETION STATUS ${todaysDDMMYYYY()}", 1);
    } else {
      _myBox.put("COMPLETION STATUS ${todaysDDMMYYYY()}", 0);
    }
  ...
  List<Workout> readFromDatabase() {
    ...
    for (int i = 0; i < workoutNames.length; i++) {
      ...
      for (int j = 0; j < exerciseDetails[i].length; j++) {
        // so add each exercise into list
...
List<List<List<String>>> convertObjectToExerciseList(List<Workout> workouts) {
  ...
  Lower Body
  [ [ squats, 25kg, 10reps, 3sets], [legraise, 30kg, 10reps, 3sets]. [ calfraise, 10kg, 10reps, 3sets]
